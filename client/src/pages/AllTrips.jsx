...
const AllTrips = () => {
  ...
  return (
    <div className="md:p-14 p-6 w-full gap-8 flex flex-col">
      ...
      <div className="w-full flex items-center flex-col md:flex-row justify-between  text-[#2E2C48] mt-6 gap-4">
        ...
        <div className="flex gap-3">
          {Array.from({ length: Math.ceil(destinations.length / 8) }).map(
            (_, index) => (
              <button
                key={index}
                onClick={() => {
                ...
                className={`cursor-pointer flex shadow flex-row items-center text-[14px] ${
                  currentPage === index + 1
                    ...
                    : "text-[#2E2C48] bg-white "
                } font-medium justify-center gap-10 h-[40px] w-[40px] rounded-md`}
              >
                {index + 1}
              </button>
            )
